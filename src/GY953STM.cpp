...
GY953STM::GY953STM(const uint8_t cs_pin,const uint8_t int_pin)
{
	_CS = cs_pin;
	_INTp = int_pin;
	_attention = false;
...
void GY953STM::begin(void)
{
	...
	_spiPort->begin();
		...
		_spiPort->setClockDivider(SPI_CLOCK_DIV64);
...
void GY953STM::begin2(SPIClass &spiPort) {
  _spiPort = &spiPort; 
  begin();
}

void GY953STM::_disableCS(void)
{
...
void GY953STM::_startSPItransaction(void)
{
	#if defined(SPI_HAS_TRANSACTION)
		_spiPort->beginTransaction(SPISettings(400000, MSBFIRST, SPI_MODE3));
	#else
		cli();//protect from interrupts
	#endif
}

void GY953STM::_endSPItransaction(void)
{
	#if defined(SPI_HAS_TRANSACTION)
...
void GY953STM::writeRegister(byte reg, byte *data, int len) 
{
	byte i = 0;
	_startSPItransaction();
	_enableCS();
	_spiPort->transfer(reg); //Send register location
	while (i < len){
		_spiPort->transfer(data[i++]);
	}
	_disableCS();
	_endSPItransaction();
}
...
void GY953STM::readRegister(byte reg, byte *data, int len) 
{
	byte i = 0;
	_startSPItransaction();
	_enableCS();
	...
	while (i < len){
		data[i++] = _spiPort->transfer(0);
	}
	_disableCS();
	_endSPItransaction();
}

void GY953STM::isr(void)
{
	if (!_attention) _attention = true;
}

void GY953STM::enableInt(void)
{
	pinMode(_INTp,INPUT);
	_attention = false;
	...
	if (theINT != NOT_AN_INTERRUPT) {
		_spiPort->usingInterrupt(theINT);
...
bool GY953STM::update(uint8_t mode)
{
	uint8_t sum = 0;
	if (_attention){
...
void GY953STM::sensorEnabled(uint8_t mode)
{
	byte tempdata;
	switch(mode){
		case 0://all
...
void GY953STM::setRefreshRate(uint8_t freq)
{
	byte tempdata;
	if (freq <= 50){
...
void GY953STM::calibration(uint8_t mode)
{
	//byte tempdata;
...
void GY953STM::setMode(uint8_t mode)
{
...
void GY953STM::getACC(int *data)
{
	if (_currentMode != 2) setMode(2);
	memcpy(data,_raw,sizeof(_raw));
}

void GY953STM::getGYR(int *data)
{
	if (_currentMode != 3) setMode(3);
	memcpy(data,_raw,sizeof(_raw));
}

void GY953STM::getMAG(int *data)
{
	if (_currentMode != 4) setMode(4);
	memcpy(data,_raw,sizeof(_raw));
}

void GY953STM::getQ(int *data)
{
	if (_currentMode != 5) setMode(5);
	memcpy(data,_raw,sizeof(_raw));
}

void GY953STM::readAccuracy(byte *data)
{
...
void GY953STM::readRange(byte *data)
{
